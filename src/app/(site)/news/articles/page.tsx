...
export default function ConnectorTypes() {
  ...
  async function getData(page: number = 1, page_size: number = 50) {
    ...
    try {
      ...
      const result = await BlogCategoryService.getList(body);
  ...
  async function deleteType() {
    setIsLoading(true)
    try {
      const result = await BlogCategoryService.deleteCategory(selectedId);
      if (result?.data?.success) {
        toast.success("The article was removed successfully!")
        setModalDelete(false)
        await getData();
      } else {
        toast.error(result?.data?.message ?? "There was a problem trying to remove the article. Please retry again!");
      }
    } catch (stsError: any) {
      toast.error(stsError?.response?.data?.message ?? "There was a problem trying to remove the article. Please retry again!");
    } finally {
      setIsLoading(false)
    }
  }
