...
export default function BlogCategories() {
  ...
  async function getData(page: number = 1, page_size: number = 50) {
    ...
    try {
      ...
      const result = await ProjectsTypeService.getList(body);
      ...
      <>
        <div className="flex justify-between px-7.5 py-4.5">
          ...
          <button
            ...
            type="button"
            onClick={() => { router.push(`/projects/type/add`) }}
          >
            <CirclePlusIcon />
            Add New Type
          </button>
  ...
  async function deleteType() {
    setIsLoading(true)
    try {
      const result = await ProjectsTypeService.deleteType(selectedId);
      if (result?.data?.success) {
  ...
  return (
    <>
      <Breadcrumb pageName="Project Types" />
