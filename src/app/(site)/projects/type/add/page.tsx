...
const AddType = () => {
  ...
  async function submitForm(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    try {
      const result = await ProjectsTypeService.createType(data);
      if (result?.data?.success) {
        toast.success("Type created successfully");
        router.push("/projects/type");
      } else {
        toast.error(result?.data?.message ?? "There was a problem trying to create the type. Please check all fields!");
      }
    } catch (err: any) {
      toast.error(err?.response?.data?.message ?? "There was a problem trying to create the type. Please check all fields!");
    } finally {
      setLoading(false);
    }
  }
